...
// For this practice problem, write a program that outputs The Flintstones Rock! 10 times, with each line indented 1 space to the
...
for (let line = 0; line < 10; line++) {
  flintstones.padStart(flintstones.length + line, ' ');
}
...
for (const char of munstersDescription) {
...
// Bonus: What is the purpose of number % divisor === 0 in that code? => To check if each number from n down to 1 evenly divides the number given
...
nanArray[0] === NaN; // false

// Bonus:
// How can you reliably test if a value is NaN? => Number.isNan()
...
function messWithDemographics(demoObject) {
  // demoObject = JSON.parse(JSON.stringify(demoObject)); => Creates a deepy copy of munsters
...
// => Yes, because an object is a reference variable and the function modifies the objects data in place 


// 9) Function and method calls can take expressions as arguments. Suppose we define a function named rps as follows, which follows the
//    classic rules of the rock-paper-scissors game, but with a slight twist: in the event of a tie, it just returns the choice made by both players.
...
rps(rps(rps("rock", "paper"), rps("rock", "scissors")), "rock"); // => paper
